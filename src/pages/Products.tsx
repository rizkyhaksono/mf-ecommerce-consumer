import { Suspense, lazy, useState } from "react"
import { useCart } from "../context/CartContext"
import { Product } from "../types"
import { useToast } from "../context/ToastContext"
import Loading from "../components/Loading"
import RemoteErrorBoundary from "../components/RemoteErrorBoundary"

const ProductGrid = lazy(() => import("productProvider/ProductGrid"))
const ProductDetails = lazy(() => import("productProvider/ProductDetails"))

const products = [
  {
    id: "1",
    name: "Wireless Bluetooth Headphones",
    price: 79.99,
    image: "https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=500&h=500&fit=crop",
    description: "Premium wireless headphones with active noise cancellation and 30-hour battery life.",
    category: "Electronics",
    rating: 4.5,
    stock: 45,
    featured: true,
  },
  {
    id: "2",
    name: "Smart Watch Pro",
    price: 299.99,
    image: "https://images.unsplash.com/photo-1523275335684-37898b6baf30?w=500&h=500&fit=crop",
    description: "Advanced fitness tracking, heart rate monitoring, and GPS navigation.",
    category: "Electronics",
    rating: 4.8,
    stock: 30,
    featured: true,
  },
  {
    id: "3",
    name: "Laptop Backpack",
    price: 49.99,
    image: "https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=500&h=500&fit=crop",
    description: "Durable and water-resistant backpack with padded laptop compartment.",
    category: "Accessories",
    rating: 4.3,
    stock: 80,
  },
  {
    id: "4",
    name: "USB-C Hub Adapter",
    price: 34.99,
    image: "https://images.unsplash.com/photo-1625948515291-69613efd103f?w=500&h=500&fit=crop",
    description: "7-in-1 USB-C hub with HDMI, USB 3.0, SD card reader, and more.",
    category: "Electronics",
    rating: 4.6,
    stock: 120,
  },
  {
    id: "5",
    name: "Mechanical Keyboard",
    price: 129.99,
    image: "https://images.unsplash.com/photo-1587829741301-dc798b83add3?w=500&h=500&fit=crop",
    description: "RGB backlit mechanical keyboard with customizable keys and switches.",
    category: "Electronics",
    rating: 4.7,
    stock: 55,
    featured: true,
  },
  {
    id: "6",
    name: "Wireless Mouse",
    price: 39.99,
    image: "https://images.unsplash.com/photo-1527864550417-7fd91fc51a46?w=500&h=500&fit=crop",
    description: "Ergonomic wireless mouse with precision tracking and long battery life.",
    category: "Electronics",
    rating: 4.4,
    stock: 95,
  },
  {
    id: "7",
    name: "Portable SSD 1TB",
    price: 149.99,
    image: "https://images.unsplash.com/photo-1531492746076-161ca9bcad58?w=500&h=500&fit=crop",
    description: "Ultra-fast portable SSD with 1TB storage and USB-C connectivity.",
    category: "Electronics",
    rating: 4.9,
    stock: 40,
  },
  {
    id: "8",
    name: "Desk Lamp LED",
    price: 45.99,
    image: "https://images.unsplash.com/photo-1507473885765-e6ed057f782c?w=500&h=500&fit=crop",
    description: "Modern LED desk lamp with adjustable brightness and color temperature.",
    category: "Home",
    rating: 4.2,
    stock: 70,
  },
  {
    id: "9",
    name: "Webcam 1080p",
    price: 69.99,
    image: "https://images.unsplash.com/photo-1588508065123-287b28e013da?w=500&h=500&fit=crop",
    description: "Full HD webcam with auto-focus and built-in noise-canceling microphone.",
    category: "Electronics",
    rating: 4.5,
    stock: 60,
  },
  {
    id: "10",
    name: "Phone Stand",
    price: 19.99,
    image: "https://images.unsplash.com/photo-1609081219090-a6d81d3085bf?w=500&h=500&fit=crop",
    description: "Adjustable aluminum phone stand compatible with all smartphones.",
    category: "Accessories",
    rating: 4.1,
    stock: 150,
  },
  {
    id: "11",
    name: "Bluetooth Speaker",
    price: 89.99,
    image: "https://images.unsplash.com/photo-1608043152269-423dbba4e7e1?w=500&h=500&fit=crop",
    description: "Portable Bluetooth speaker with 360° sound and waterproof design.",
    category: "Electronics",
    rating: 4.6,
    stock: 75,
    featured: true,
  },
  {
    id: "12",
    name: "Monitor Stand",
    price: 54.99,
    image: "https://images.unsplash.com/photo-1527689368864-3a821dbccc34?w=500&h=500&fit=crop",
    description: "Ergonomic monitor stand with adjustable height and storage space.",
    category: "Accessories",
    rating: 4.3,
    stock: 50,
  },
]

const Products = () => {
  const { addToCart } = useCart()
  const { showToast } = useToast()
  const [selectedProduct, setSelectedProduct] = useState<Product | null>(null)

  const handleAddToCart = (product: Product, quantity = 1) => {
    addToCart(product, quantity)
    setSelectedProduct(null)
    showToast(`Added ${quantity}× ${product.name} to cart`, "success")
  }

  const handleViewDetails = (product: Product) => {
    setSelectedProduct(product)
  }

  return (
    <div className="min-h-screen bg-zinc-50">
      <div className="container mx-auto px-4 py-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-zinc-900 mb-2">Our Products</h1>
          <p className="text-zinc-500 text-sm">Discover our curated collection of quality products</p>
        </div>

        <RemoteErrorBoundary name="Product Provider">
          <Suspense fallback={<Loading />}>
            <ProductGrid products={products} onAddToCart={handleAddToCart} onViewDetails={handleViewDetails} />
          </Suspense>
        </RemoteErrorBoundary>

        {selectedProduct && (
          <RemoteErrorBoundary name="Product Provider">
            <Suspense fallback={<Loading />}>
              <ProductDetails product={selectedProduct} onAddToCart={handleAddToCart} onClose={() => setSelectedProduct(null)} />
            </Suspense>
          </RemoteErrorBoundary>
        )}
      </div>
    </div>
  )
}

export default Products
